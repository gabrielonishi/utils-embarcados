...
// Config ButOnOff
#define BUTOF_PIO PIOA
#define BUTOF_PIO_ID ID_PIOA
#define BUTOF_PIO_IDX 21
#define BUTOF_PIO_IDX_MASK (1u << BUTOF_PIO_IDX)
...
SemaphoreHandle_t xSemaphoreButOF;
...
void butof_callback(void) {
	xSemaphoreGiveFromISR(xSemaphoreButOF, 0);
}
...
//     init_but(BUT2_PIO, BUT2_PIO_ID, BUT2_PIO_IDX_MASK);
	init_but(BUTOF_PIO, BUTOF_PIO_ID, BUTOF_PIO_IDX_MASK);
//     init_IRQ_OF();
    ...
    while (1) {
        printf("%d", pio_get(BUTOF_PIO, PIO_INPUT, BUTOF_PIO_IDX_MASK));
//         printf("%d", pio_get(BUT_PIO, PIO_INPUT, BUT_PIO_IDX_MASK));
...
void init_IRQ_OF(void) {
	pio_handler_set(BUTOF_PIO,
	BUTOF_PIO_ID,
	BUTOF_PIO_IDX_MASK,
	PIO_IT_EDGE,
	butof_callback);

	pio_enable_interrupt(BUTOF_PIO, BUTOF_PIO_IDX_MASK);
	pio_get_interrupt_status(BUTOF_PIO);

	NVIC_EnableIRQ(BUTOF_PIO_ID);
	NVIC_SetPriority(BUTOF_PIO_ID, 4);
}
...
int main(void) {
	...
	xSemaphoreButOF = xSemaphoreCreateBinary();
    ...
    if (xSemaphoreButOF == NULL) {
	    printf("Falha em criar xSemaphoreButOF \n");
	    } else {
	    printf("xSemaphoreButOF criado \n");
    }
